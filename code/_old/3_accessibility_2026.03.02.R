...
p_load(tidyverse, rstudioapi, r5r, sf, data.table, tmap, viridisLite # ggplot2, interp, h3jsr, TODO delete 
       )
...
DEPARTURE_DATETIME_OLD <- as.POSIXct("18-02-2026 8:00:00",
                                     format = "%d-%m-%Y %H:%M:%S")
DEPARTURE_DATETIME_NEW <- as.POSIXct("25-02-2026 8:00:00",
                                     format = "%d-%m-%Y %H:%M:%S")
PERCENTILES <- c(50) # median accessibility only

# origins
ORIGIN_IDS <- c("Orange St/Media", "Sharon Hill", 
                "Drexel Hill Junction", "69th St Transit center") # TODO fix
...
# TRACTS_PATH <- "./input/Census TIGER Line Shapefiles/tl_2025_42_tract/tl_2025_42_tract.shp" TODO delete
...
# TODO delete
# tmap_mode("plot")
# tm_shape(ttm_old_sf # %>% filter(from_id == "Orange St/Media")
#          ,
#          bbox = bounding_box
#          ) +
...
# 
# tm_shape(ttm_new_sf # %>% filter(from_id == "Orange St/Media")
#          ,
#          bbox = bounding_box
# ) +
#   tm_dots(fill = "from_id",
...
rr_routes <- subset(transit_net$routes, mode %like% 'RAIL')

# remove diversion routes
trolley_diversion_filters <- tibble(id = c("40th/Market", "40th/Walnut", "49th/Greenway"),
                                    lon = c(-75.20186440305577, -75.20259204009699, -75.2138537663788),
                                    lat = c(39.95788702742263, 39.95414202021738, 39.94193301079272)) %>%
...
metro_order <- c("D1", "D2", "B1", "B2", "B3", "L1", "M1", "T1", "T2", "T3", "T4", "T5", "G1")
metro_routes <- metro_routes %>%
  mutate(route_id = factor(route_id, levels = metro_order)) %>%
  arrange(desc(route_id))

# origins
origin_id <- "Sharon Hill"
origin <- ORIGINS %>%
  filter(id == origin_id) %>%
  st_as_sf(coords = c("lon", "lat"), crs = GCS)
...
# map
ttm_comp_sub <- ttm_comp %>% 
  filter(from_id == origin_id) %>%
  group_by(change) %>%
  summarize(do_union = TRUE) %>%
  ungroup()
tmap_mode("plot")
tm_shape(rr_routes, bbox = bounding_box) +
  tm_lines(col = "#4C748C", lwd = 1) + 
tm_shape(metro_routes) +
  tm_lines(col = "route_id",
           col.scale = tm_scale_categorical(values = metro_palette),
           col.legend = tm_legend(show = FALSE),
           lwd = 2) +
  tm_shape(ttm_comp_sub#, bbox = bounding_box
  ) +
